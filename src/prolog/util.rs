...
impl<'a> ContextExt<'a> for Context<'a, ActivatedEngine<'a>> {
  ...
  fn assert(&'a self, term:&Term<'a>, module:Option<&'a Module>, location:AssertLocation)
    ...
    let flags = match location {
      | AssertLocation::First => fli::PL_ASSERTA,
      | AssertLocation::Last => fli::PL_ASSERTZ,
    };
    ...
    // as well.
    let res = unsafe { fli::PL_assert(term.term_ptr(), module, flags as i32) };
    if res == fli::TRUE as i32 {
      Ok(())
    } else if res == fli::FALSE as i32 {
  ...
  fn canonical(&'a self, term:&Term<'a>) -> Option<String> {
    ...
    let result = unsafe {
      fli::PL_get_nchars(term.term_ptr(),
                         (&mut len as *mut usize).cast::<fli::size_t>(),
                         &mut s,
                         flags)
    };
    if result == 0 {
      ...
      let string = std::str::from_utf8(slice).expect("utf-8").to_string();
